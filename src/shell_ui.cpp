...
class SystemClock {
public:
  void draw(const Config &cfg, const ImGuiViewport *vp, bool left_side,
            bool bottom) {
...
public:
  void draw(const ShellState &state, const Config &cfg,
    ...
    bool light = (cfg.theme == "light");
    ImU32 col =
        light ? IM_COL32(40, 40, 45, 220) : IM_COL32(235, 235, 235, 220);
    ...
    for (size_t i = 0; i < segs.size(); ++i) {
      ...
      if (s.tex) {
        ...
        dl->AddImage((ImTextureID)s.tex, ImVec2(cx, iy),
                     ImVec2(cx + hs, iy + hs), ImVec2(0, 0), ImVec2(1, 1),
...
public:
  void draw(const ShellState &state, const Config &cfg,
    ...
    ImU32 idle =
        light ? IM_COL32(40, 40, 45, 220) : IM_COL32(220, 222, 228, 220);
...
public:
  void draw(ShellState &state, const Config &cfg, const ShellActions &actions,
    ...
    static const char *labels[4] = {"SETTINGS", "CONTROLLERS",
    ...
    for (int m = 0; m < 4; ++m) {
      ...
      if (state.menu_open) {
        ...
        if (active) {
          col =
              light ? IM_COL32(180, 190, 210, 255) : IM_COL32(94, 94, 100, 255);
        } else if (hovered) {
          col =
              light ? IM_COL32(215, 225, 235, 255) : IM_COL32(54, 54, 60, 255);
...
static PanelLayout beginPanel(const char *title, int list_count,
                              int footer_rows, const Config &cfg) {
  PanelLayout L;
...
public:
  void draw(ShellState &st, Config &cfg, const ShellActions &actions,
    ...
    for (int i = 0; i < list_count; ++i) {
      ...
      dl->AddText(ImVec2(lx, rmin.y + (L.row_h - tt.y) * 0.5f), L.text_main,
                  rows[i].label);
...
public:
  void draw(ShellState &st, Config &cfg, const ShellActions &actions) {
    ...
    for (int i = 0; i < list_count; ++i) {
      ...
      ImGui::SetCursorScreenPos(rmin);
      ImGui::InvisibleButton("##crow",
                             ImVec2(rmax.x - rmin.x, rmax.y - rmin.y));
      bool hovered = ImGui::IsItemHovered();
      if (ImGui::IsItemClicked()) {
        if (picking)
      ...
      dl->AddText(
          ImVec2(L.content_min.x + L.pad, rmin.y + (L.row_h - tt.y) * 0.5f),
    ...
    if (focus == list_count)
      dl->AddRectFilled(bmin, bmax, L.row_focus, 6);
    else if (hovered)
      dl->AddRectFilled(bmin, bmax, L.row_hover, 6);
...
public:
  void draw(ShellState &state, const Config &cfg, const ShellActions &actions,
    ...
    float icon_sel = cross_axis * cfg.icon_sel_pct *
                     (horizontal ? 1.5f : cfg.icon_vert_scale);
    ...
    for (const Row &row : rows) {
      ...
      ImVec2 ts = g_font_tile ? g_font_tile->CalcTextSizeA(
                                    fs, 10000.0f, 0.0f, label.c_str(), nullptr)
                              : ImGui::CalcTextSize(label.c_str());

      float isz = flerp(icon_un, icon_sel, t);

      // Prioridad: text_color > icon_tint > blanco
      ImU32 label_col;
...
// Helper function needed by TileCarousel
...
static EdgeFades g_edge_fades;
static HelpHints g_help_hints;
static PlayerIndicators g_player_indicators;
static SystemMenu g_system_menu;
static SettingsPanel g_settings_panel;
static ControllersPanel g_controllers_panel;
static TileCarousel g_tile_carousel;
...
void drawShellImGui(ShellState &state, const Config &cfg,
  ...
  if (panel_open) {
    if (state.show_controllers)
      ...
      g_settings_panel.draw(state, const_cast<Config &>(cfg), actions,
                            state.show_settings);
  } else {
    if (state.menu_anim > 0.001f) {
      dl->AddRectFilled(ImVec2(0, 0), ImVec2(W, H),
