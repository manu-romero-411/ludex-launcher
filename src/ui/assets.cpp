...
void loadAllWallpapers(Renderer &renderer, ShellState &state, const Config &cfg,
    ...
    for (const auto &path : candidates) {
        WallpaperLayer L;
        int w = 0, h = 0;
        void* raw = renderer.loadTextureFromFile(path, &w, &h, 0, nullptr,
                                                  screen_w, screen_h);
        if (raw) {
    ...
    for (auto &app : state.apps) {
        if (!app.icon_path.empty()) {
            app.icon_texture = TexturePtr(
                renderer.loadTextureFromFile(app.icon_path, nullptr, nullptr, icon_max,
...
void loadUiIcons(Renderer &renderer, ShellState &state, const Config &cfg,
    ...
    if (cfg.icons_dir.empty()) {
        ic.settings.reset(); ic.exit.reset(); ic.shutdown.reset();
        ic.restart.reset(); ic.suspend.reset(); ic.nav_v.reset();
        ic.nav_h.reset(); ic.accept.reset(); ic.back.reset();
        ic.home.reset(); ic.gamepad.reset();
        return;
    }
    ...
    if (cfg.help_icons != HelpIcons::None) {
        ...
        ic.nav_v.reset(); ic.nav_h.reset(); ic.accept.reset();
        ic.back.reset(); ic.home.reset();
    }

    int loaded = 0;
    const TexturePtr* ptrs[] = {&ic.settings, &ic.exit, &ic.shutdown, &ic.restart,
                                &ic.suspend, &ic.nav_v, &ic.nav_h, &ic.accept,
                                &ic.back, &ic.home};
