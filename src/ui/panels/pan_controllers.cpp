...
PanelSpec makeControllersPanelSpec(ShellState& st, Config& cfg, const ShellActions& actions) {
    ...
    if (!picking) {
        ...
        spec.title = "PAD FOR PLAYER " + std::to_string(st.controller_pick_player + 1);
        spec.focus_ptr = &st.controller_pick_focus;
        spec.on_back = [](ShellState& s, Config&) { s.controller_pick_player = -1; };
        ...
        default_row.on_select = [&st, &cfg, &actions](ShellState& s, Config& c, const ShellActions& a) {
            int p = s.controller_pick_player;
            c.controller_guid[p].clear();
            c.controller_name[p].clear();
            if (a.apply_controllers) a.apply_controllers();
            c.save(c.ini_path);
