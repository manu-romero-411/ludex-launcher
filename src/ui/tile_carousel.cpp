...
void TileCarousel::draw(ShellState& state, const Config& cfg, const ShellActions& actions,
    ...
    float icon_un = cross_axis * cfg.icon_pct * (horizontal ? 1.5f : cfg.icon_vert_scale);
    float icon_sel = cross_axis * cfg.icon_sel_pct * (horizontal ? 1.5f : cfg.icon_vert_scale);
    ...
    for (const Row& row : rows) {
        ...
        if (accept_tile_input) {
            ...
            if (ImGui::IsItemClicked()) {
                if (t > 0.9f) state.pending_launch = row.idx;
        ...
        if (tile_hovered && !tile_active) {
            ...
            ImU32 hover_border = light ? IM_COL32(100, 120, 200, 180) : IM_COL32(150, 170, 255, 180);
        ...
        ImVec2 ts = g_font_tile ? g_font_tile->CalcTextSizeA(fs, 10000.0f, 0.0f, label.c_str(), nullptr)
                                : ImGui::CalcTextSize(label.c_str());
        float isz = flerp(icon_un, icon_sel, t);

        ImU32 label_col;
        if (app.has_text_color) {
            label_col = IM_COL32(app.text_color.r, app.text_color.g, app.text_color.b, 255);
        } else if (app.has_icon_tint) {
            label_col = IM_COL32(app.icon_tint.r, app.icon_tint.g, app.icon_tint.b, 255);
        } else {
        ...
        if (horizontal) {
            ...
            dl->AddText(g_font_tile, fs, ImVec2(label_x, label_y), label_col, label.c_str());
        } else {
            if (app.icon_texture) {
                ImVec2 imin(x0 + pad, row.main_pos - isz * 0.5f);
                dl->AddImage((ImTextureID)app.icon_texture, imin, ImVec2(imin.x + isz, imin.y + isz));
            }
