...
bool light = isLight(cfg.theme);
    ...
    static const char* labels[4] = {"SETTINGS", "CONTROLLERS", "QUIT TO DESKTOP", "SHUTDOWN..."};
    ...
    for (int m = 0; m < 4; ++m) {
        ...
        if (icons[m]) {
            dl->AddImage((ImTextureID)icons[m],
                         ...
                         ImVec2(lx + isz, y0 + (menu_h + isz) * 0.5f),
                         ImVec2(0, 0), ImVec2(1, 1), text_col);
            lx += isz + W * 0.010f;
        }
        dl->AddText(ImVec2(lx, y0 + (menu_h - ts.y) * 0.5f), text_col, label.c_str());
        ImGui::PopFont();
    }
}
