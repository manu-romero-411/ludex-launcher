...
struct UiIcons {
  ...
  TexturePtr os;
  TexturePtr desktop;
  TexturePtr git_commit;
  TexturePtr build_date;
  ...
  void *byIndex(RowIcon icon) const {
    ...
    case RowIcon::Os:
      return os.get();
    case RowIcon::Desktop:
      return desktop.get();
    case RowIcon::GitCommit:
      return git_commit.get();
    case RowIcon::BuildDate:
      return build_date.get();
...
struct ShellState {
  ...
  bool anyPanelOpen() const {
    return show_settings || show_power || show_controllers || show_bluetooth ||
           show_bluetooth_scan || show_system || show_about;
  }
  ...
  void nav(int dy);
  void update(float dt, const Config &cfg, Renderer &renderer, int screen_w,
              int screen_h);
