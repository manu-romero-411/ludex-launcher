...
static SDL_Surface *loadSvgSurface(const std::filesystem::path &path, int tw,
    ...
    for (int y = 0; y < h; ++y) {
        ...
        for (int x = 0; x < w; ++x) {
            unsigned int b = row[x * 4 + 0], g = row[x * 4 + 1],
...
/* Carga el surface base (SVG o raster). Solo CPU: seguro desde cualquier
hilo (no toca SDL_Renderer ni el subsistema de vídeo). */
static SDL_Surface *loadBaseSurface(const std::filesystem::path &path,
                                    int svg_target) {
    ...
    if (ext == ".svg") {
        int target = svg_target > 0 ? svg_target : 128;
#ifdef LUDEX_HAVE_RSVG
...
static void resizeSurfaceTo(SDL_Surface *&rgba, int dw, int dh) {
    if (!rgba || (dw == rgba->w && dh == rgba->h))
        return;
    SDL_Surface *resized =
...
public:
    ...
    void *loadTextureFromFile(const std::filesystem::path &path, int *out_w,
        ...
        SDL_Surface *surface = loadBaseSurface(path, max_dim);
        if (!surface) {
        ...
        if (cover_w > 0 && cover_h > 0) {
            ...
            resizeSurfaceTo(rgba, std::max(1, (int)std::lround(rgba->w * s)),
    ...
    bool decodeImage(const std::filesystem::path &path, int cover_w,
                     int cover_h, DecodedImage &out) override {
        SDL_Surface *surface =
            loadBaseSurface(path, std::max(cover_w, cover_h));
        if (!surface)
            return false;
        SDL_Surface *rgba =
            SDL_ConvertSurfaceFormat(surface, SDL_PIXELFORMAT_RGBA32, 0);
        ...
        out.w = rgba->w;
        out.h = rgba->h;
        const size_t nbytes = (size_t)rgba->h * (size_t)rgba->pitch;
        out.pixels.resize(nbytes);
        std::memcpy(out.pixels.data(), rgba->pixels, nbytes);
        SDL_FreeSurface(rgba);
        return true;
    }

    void *createTextureFromPixels(const uint8_t *pixels, int w,
                                  int h) override {
        if (!pixels || w <= 0 || h <= 0)
            return nullptr;
        SDL_Surface *surf = SDL_CreateRGBSurfaceWithFormatFrom(
            (void *)pixels, w, h, 32, w * 4, SDL_PIXELFORMAT_RGBA32);
        if (!surf)
            return nullptr;
        ...
        if (tex) {
            ...
            SDL_Log("createTextureFromPixels falló: %s", SDL_GetError());
        }
        return tex;
    }
