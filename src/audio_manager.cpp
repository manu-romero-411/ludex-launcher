...
void AudioManager::loadMusicFromDirectory(const std::filesystem::path& dir) {
    ...
    std::cout << "[ludex] Loaded " << music_files_.size() << " music tracks from " << dir << "\n";
}
...
void AudioManager::playRandomTrack() {
    ...
    do {
        ...
        attempts++;
    } while (new_track == current_track_ && music_files_.size() > 1 && attempts < 10);

    current_track_ = new_track;
    current_music_ = Mix_LoadMUS(music_files_[current_track_].c_str());

    if (!current_music_) {
        std::cerr << "[ludex] Failed to load music: " << music_files_[current_track_] 
                  << " - " << Mix_GetError() << "\n";
        return;
    }
