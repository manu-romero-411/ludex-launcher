...
void BluetoothManager::parseDevicesOutput(const std::string &output,
                                          std::vector<BluetoothDevice> &target) {
    target.clear();
...
void BluetoothManager::refreshConnectedState() {
    for (auto &dev : devices_) {
        std::string cmd =
            "bluetoothctl info " + dev.mac + " 2>/dev/null | grep 'Connected: yes'";
        std::string out = runCmd(cmd);
        dev.connected = !out.empty();
    }
}
...
void BluetoothManager::startScan(int timeout_sec) {
    ...
    if (pid == 0) {
        ...
        execlp("bluetoothctl", "bluetoothctl", "--timeout", t.c_str(),
               "scan", "on", nullptr);
        _exit(127);
    }
...
void BluetoothManager::update() {
    ...
    if (scanning_) {
        double elapsed =
            std::chrono::duration<double>(now - scan_start_).count();
